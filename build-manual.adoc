...
It is a convinient alternative to _corresponding_depends_ useful in cases
dealing with Java.
Other languag-level extensions are easy to create and may be included in the
_build.lisp_ file.
...
This command is used in recipies in order to run system-level commands
such as runninga compiler.
